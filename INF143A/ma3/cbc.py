...
#splits the plaintext into blocks of length L, padding with 0's if necessary
def split_into_blocks(P,block_size):
    padding_value = block_size - (len(P) % block_size)
    padding = bytes([padding_value] * padding_value)
    return P + padding
...
def cbc_enc(P,K,IV):
    ...
    for block in blocks:
        xor_enc = XOR(previous_block,block)
        ciphertext_array.append(encrypt(xor_enc,K))
        previous_block = block
    return mergeBlocks(ciphertext_array)
